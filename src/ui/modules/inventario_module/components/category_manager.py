...
class CategoryManagerWidget(InventoryManagerBase):
    ...
    def update_categories_table(self) -> None:
        ...
        try:
            ...
            categorias_activas = [cat for cat in self.categorias_cache if cat.get('activa', True)]
            categorias_inactivas = [cat for cat in self.categorias_cache if not cat.get('activa', True)]
            ...
            for row, categoria in enumerate(categorias_a_mostrar):
                ...
                self.table.setItem(
                    row, 0, QTableWidgetItem(str(virtual_id))
                )
    ...
    def add_new_category(self) -> None:
        ...
        try:
            ...
            QMessageBox.warning(
                self,
                "Error",
                f"No se pudo agregar la categoría: {e}"
            )
    ...
    def edit_selected_category(self) -> None:
        ...
        try:
            ...
            if not categoria_id:
                QMessageBox.warning(self, "Error", "No se pudo obtener el ID de la categoría")
                return
                
            categoria = next(
                (c for c in self.categorias_cache if c.get('id') == categoria_id),
                None
            )
            ...
            QMessageBox.warning(
                self,
                "Error",
                f"No se pudo editar la categoría: {e}"
            )
    ...
    def delete_selected_category(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Intentando eliminar categoría ID: {categoria_id}, Nombre: {nombre}")
            ...
            if reply == QMessageBox.StandardButton.Yes:
                logger.debug(f"Usuario confirmó eliminación de categoría ID: {categoria_id}")
...
class CategoryDialog(InventoryDialogBase):
    ...
    def _load_category_data(self):
        ...
        if self.categoria_data:
            ...
            self.description_edit.setPlainText(str(self.categoria_data.get('descripcion', '')))
