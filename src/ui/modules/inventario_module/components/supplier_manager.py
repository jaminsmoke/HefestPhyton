...
class SupplierDialog(InventoryDialogBase):
    ...
    def _create_contact_info_form(self):
        ...
        phone_label, phone_widget = self.create_form_field("Teléfono", self.phone_edit)
        form.addRow(phone_label, phone_widget)

        # Email
        self.email_edit = QLineEdit()
        email_label, email_widget = self.create_form_field("Email", self.email_edit)
    ...
    def _create_config_form(self):
        ...
        notes_label, notes_widget = self.create_form_field("Notas", self.notes_edit)
...
class SupplierManagerWidget(InventoryManagerBase):
    ...
    def update_suppliers_table(self) -> None:
        ...
        try:
            ...
            proveedores_activos = [
                prov
                for prov in self.suppliers_cache
                if prov.get("activo", True)
                or prov.get("estado", "").lower() == "activo"
            ]
            ...
            for row, proveedor in enumerate(proveedores_activos):
                ...
                # Email
                self.table.setItem(row, 4, QTableWidgetItem(proveedor.get("email", "")))
            ...
            logger.info(
                f"Tabla de proveedores actualizada con {len(proveedores_activos)} elementos"
            )
    ...
    def delete_item(self):
        ...
        selected = self.get_selected_item()
        if not selected:
            self.show_warning("Por favor selecciona un proveedor para eliminar")
            return
