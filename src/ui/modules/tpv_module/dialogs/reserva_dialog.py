...
class ReservaDialog(QDialog):
    ...
    def __init__(
        ...
        print(
            f"[ReservaDialog] __init__ llamado. mesa={getattr(mesa, 'id', None)}, modo_edicion={modo_edicion}"
        )
        ...
        print(
            f"[ReservaDialog] connect_signals ejecutado. self.mesa={getattr(self.mesa, 'id', None)}"
        )
    ...
    def setup_reserva_section(self, layout):
        ...
        self.hora_feedback_label.setSizePolicy(
    ...
    def setup_detalles_section(self, layout):
        ...
        self.notas_input.setPlaceholderText(
            "Alergias, preferencias, ocasión especial, etc..."
        )
    ...
    def setup_footer(self, layout):
        ...
        self.cancelar_btn.setSizePolicy(
        ...
        if self.modo_edicion:
            ...
            self.guardar_btn.setSizePolicy(
                QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed
            )
            ...
            self.eliminar_btn.setSizePolicy(
                QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed
            )
            ...
            self.aceptar_btn.setSizePolicy(
                QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed
            )
    ...
    def validar_hora_reserva(self):
        ...
        if self.reserva_service:
            ...
            reservas_activas = [
                ...
                or (
                    getattr(r, "fecha_reserva", None) == fecha - td(days=1)
        ...
        if nueva_inicio < datetime.now():
            self.hora_feedback_label.setText(
                "⚠️ La fecha y hora seleccionadas ya han pasado. Elige una hora futura o pulsa 'Sugerir otra hora'."
            )
    ...
    def sugerir_proxima_hora_libre(self):
        ...
        if self._proxima_hora_libre is not None:
            ...
            if sugerida < datetime.now():
                ...
                try:
                    self.hora_input.setTime(
                        QTime(
                            self._proxima_hora_libre.hour,
                            self._proxima_hora_libre.minute,
                        )
                    )
    ...
    def validar_y_aceptar(self):
        ...
        if not self.telefono_input.text().strip():
            QMessageBox.warning(
                self, "Campo requerido", "El teléfono de contacto es obligatorio."
            )
    ...
    def cargar_datos_reserva(self, reserva):
        ...
        if reserva.fecha_reserva:
            self.fecha_input.setDate(
                QDate(
                    reserva.fecha_reserva.year,
                    reserva.fecha_reserva.month,
                    reserva.fecha_reserva.day,
                )
            )
    ...
    def cancelar_reserva(self):
        ...
        if self.reserva_service and self.reserva:
            ...
            if exito:
                QMessageBox.information(
                    self, "Reserva cancelada", "La reserva fue cancelada correctamente."
                )
