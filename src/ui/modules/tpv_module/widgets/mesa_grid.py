...
class MesaGridWidget(QWidget):
    ...
    def _create_mesa_widgets(self):
        ...
        for mesa in self.mesas:
            ...
            mesa_widget.mesa_clicked.connect(self._on_mesa_clicked)
            mesa_widget.mesa_status_changed.connect(self._on_mesa_status_changed)
            
            # Doble clic
            def make_double_click_handler(m):
                def handler():
                    self.mesa_double_clicked.emit(m)
                return handler
            
            # Simular doble clic con timer (workaround para QPushButton)
            mesa_widget.clicked.connect(self._on_mesa_single_click)
    ...
    def _on_mesa_clicked(self, mesa: Mesa):
        """Maneja el clic simple en una mesa"""
