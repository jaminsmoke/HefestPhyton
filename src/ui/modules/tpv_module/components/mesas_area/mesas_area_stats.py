...
def create_subcontenedor_metric_cards(instance: Any) -> Any:
    ...
    from PyQt6.QtWidgets import QFrame, QVBoxLayout, QHBoxLayout, QLabel, QGridLayout, QPushButton, QDialog, QCheckBox, QDialogButtonBox
...
def update_stats_from_mesas(instance: Any) -> None:
    ...
    ocupadas: int = len([mesa for mesa in instance.mesas if getattr(mesa, 'estado', None) == 'ocupada'])
...
def update_ultra_premium_stats(instance: Any) -> None:
    ...
    mesas_reservadas: int = len([m for m in instance.mesas if hasattr(m, 'estado') and getattr(m, 'estado', None) == 'reservada'])
...
def create_compact_stats(instance: Any, layout: Any) -> None:
    ...
    for icon, label, value, color in stats_config:
        stat_widget: Any = KPIWidget(icon=icon, label=label, value=value, color=color, bg_color="#fff")
        instance.compact_stats_widgets.append({  # type: ignore[reportUnknownMemberType]
            'widget': stat_widget,
            'type': label.lower(),
            'icon': icon,
            'label': label
...
def update_compact_stats(instance: Any, zonas: Any, total: Any, libres: Any, ocupadas: Any) -> None:
