...
class TPVAvanzado(QWidget):
    ...
    def __init__(self, mesa: Optional[Mesa] = None, tpv_service: Optional[TPVService] = None, db_manager=None, parent=None):
        super().__init__(parent)
        self.mesa = mesa
        if db_manager is None:
            raise ValueError("db_manager es obligatorio y debe ser inyectado explícitamente en TPVAvanzado")
        logger.info(f"[TPVAvanzado] db_manager recibido en constructor: {type(db_manager)} - {db_manager}")
        if tpv_service:
            ...
            logger.info(f"[TPVAvanzado] TPVService creado con db_manager: {self.tpv_service.db_manager}")
    ...
    def nuevo_pedido(self):
        ...
        if self.mesa and self.tpv_service:
            ...
            self.current_order = self.tpv_service.crear_comanda(self.mesa.id, usuario_id=usuario_id)
            logger.info(f"Nuevo pedido iniciado para mesa {self.mesa.numero} por usuario {usuario_id}")
            self.refrescar_estado_pedido_ui()
    ...
    def procesar_pago(self):
        ...
        if self.current_order and self.mesa:
            ...
            if usuario_id and hasattr(usuario_id, 'id'):
                ...
                usuario_id = -1  # Manejar caso sin usuario seleccionado
            comanda_id = self.current_order.id if self.current_order.id is not None else -1
            ...
            logger.info(f"Pedido completado - Mesa {self.mesa.numero}: €{total:.2f} (usuario {usuario_id})")
