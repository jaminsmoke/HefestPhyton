...
class AdvancedTPVModule(BaseModule):
    ...
    def create_order_panel(self):
        ...
        process_payment_btn.setStyleSheet(
            "background: #10b981; color: white; padding: 10px; border-radius: 6px;"
        )
    ...
    def update_order_display(self):
        ...
        for row, item in enumerate(self.current_order["items"]):
            ...
            self.order_table.setItem(
                row, 3, QTableWidgetItem(f"€{item['discount']:.2f}")
            )
        ...
        # Actualizar orden
        self.current_order.update({"subtotal": subtotal, "tax": tax, "total": total})
    ...
    def apply_discount(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if discount_type == "percentage":
                self.current_order["discount"] = self.current_order["subtotal"] * (
                    discount_value / 100
                )
            else:  # amount
                self.current_order["discount"] = min(
                    discount_value, self.current_order["subtotal"]
                )
    ...
    def process_payment(self):
        ...
        QMessageBox.information(
            self, "Procesar Pago", "Funcionalidad de pago en desarrollo."
        )
...
class DiscountDialog(QDialog):
    ...
    def get_discount(self):
        ...
        discount_type = (
            "percentage" if self.discount_type.currentIndex() == 0 else "amount"
        )
