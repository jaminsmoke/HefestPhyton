...
class MesaController(QObject):
    ...
    # Señales
    mesas_updated = pyqtSignal(list)  # Lista de mesas actualizada
    mesa_created = pyqtSignal(Mesa)   # Nueva mesa creada
    mesa_updated = pyqtSignal(Mesa)   # Mesa actualizada
    mesa_deleted = pyqtSignal(int)    # Mesa eliminada (ID)
    ...
    def editar_mesa(self, mesa_id: int, numero: int, capacidad: int, zona: str) -> bool:
        ...
        try:
            ...
            self.mesa_updated.emit(mesa_actual)
            self.mesas_updated.emit(self.mesas)
