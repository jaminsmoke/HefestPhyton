...
class HospederiaTab(BaseModule):
    ...
    def create_module_header(self):
        ...
        self.occupied_label.setStyleSheet("color: #ef4444; font-weight: bold; margin-right: 15px;")
        stats_layout.addWidget(self.occupied_label)
        
        self.cleaning_label = QLabel("Limpieza: 0")
        self.cleaning_label.setStyleSheet("color: #f59e0b; font-weight: bold; margin-right: 15px;")
    ...
    def setup_ui(self):
        ...
        self.reservations_table.setHorizontalHeaderLabels(["ID", "Huésped", "Fecha Check-In", "Fecha Check-Out", "Estado"])
        ...
        self.load_reservations = self.load_reservations or self._placeholder_load_reservations
    ...
    def create_rooms_tab(self):
        ...
        rooms_title.setStyleSheet("font-size: 18px; font-weight: bold; margin-bottom: 10px;")
    ...
    def populate_rooms_grid(self, grid_layout):
        ...
        for room_number, status, color in rooms:
            ...
            """)
            room_btn.clicked.connect(lambda checked, room=room_number: self.show_room_details(room))
    ...
    def update_room_statistics(self):
        ...
        if hasattr(self, 'available_label'):
            ...
            occupied_count = len([r for r in self.rooms_data if r.get('status') == 'occupied'])
            cleaning_count = len([r for r in self.rooms_data if r.get('status') == 'cleaning'])
    ...
    def update_reservations_table(self):
        ...
        if hasattr(self, 'reservations_table'):
            ...
            for row, reservation in enumerate(self.reservations_data):
                self.reservations_table.setItem(row, 0, QTableWidgetItem(str(reservation.get('id', ''))))
                self.reservations_table.setItem(row, 1, QTableWidgetItem(reservation.get('client_name', '')))
                self.reservations_table.setItem(row, 2, QTableWidgetItem(reservation.get('room_number', '')))
                self.reservations_table.setItem(row, 3, QTableWidgetItem(reservation.get('check_in_date', '')))
                self.reservations_table.setItem(row, 4, QTableWidgetItem(reservation.get('check_out_date', '')))
...
class NewReservationDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.room_combo.addItems(["101", "102", "103", "104", "201", "202", "203", "204"])
