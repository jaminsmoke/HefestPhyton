...
class AdminMetricsSection(QWidget):
    ...
    def setup_ui(self):
        ...
        # Crear tarjetas RESPONSIVE con mejor diferenciación
        for i, (icon, title, value, subtitle, trend, color) in enumerate(metrics_data):
            ...
            card.setMinimumSize(240, 140)  # Tamaño mínimo reducido
        ...
        for col in range(3):
            ...
            grid_layout.setColumnMinimumWidth(col, 240)  # Ancho mínimo reducido
            
        for row in range(2):
            grid_layout.setRowStretch(row, 1)  # Todas las filas se expanden igual
            grid_layout.setRowMinimumHeight(row, 140)  # Alto mínimo reducido
