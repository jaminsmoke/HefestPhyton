...
class ConfiguracionModule(BaseModule):
    ...
    def create_modules_tab(self):
        ...
        _ = [
            ...
            ("Hospedería", "Activo", "v1.3", True),            ("Inventario", "Activo", "v1.2", True),
    ...
    def cargar_configuracion(self):
        ...
        try:
            _ = self.config_manager.get_all_config()
            ...
            if idioma in [self.idioma_combo.itemText(i) for i in range(self.idioma_combo.count())]:
                self.idioma_combo.setCurrentText(idioma)
            
            moneda = config.get('currency', 'EUR (€)')
            if moneda in [self.moneda_combo.itemText(i) for i in range(self.moneda_combo.count())]:
            ...
            if tema in [self.tema_combo.itemText(i) for i in range(self.tema_combo.count())]:
            ...
            QMessageBox.warning(self, "Error", f"Error al cargar la configuración:\n{str(e)}")
    ...
    def restaurar_configuracion(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.critical(self, "Error", f"Error al restaurar la configuración:\n{str(e)}")
    ...
    def verificar_integridad(self):
        ...
        QMessageBox.information(self, "Verificación Completa", "La integridad de la base de datos es correcta.")
    ...
    def crear_backup(self):
        ...
        try:
            QMessageBox.information(self, "Respaldo Creado", "Respaldo creado exitosamente en la carpeta configurada.")
            logger.info("Respaldo manual creado")
        except Exception as e:
            logger.error("Error al crear respaldo: %s", e)
            QMessageBox.critical(self, "Error", f"Error al crear el respaldo:\n{str(e)}")
    ...
    def restaurar_backup(self):
        ...
        if file_path:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                try:
                    QMessageBox.information(self, "Restauración Completa", "Base de datos restaurada correctamente.")
                    logger.info("Base de datos restaurada desde: %s", file_path)
                except Exception as e:
                    logger.error("Error al restaurar respaldo: %s", e)
                    QMessageBox.critical(self, "Error", f"Error al restaurar el respaldo:\n{str(e)}")
