...
class SupplierManagerWidget(QWidget):
    ...
    def create_suppliers_table(self) -> QTableWidget:
        ...
        # Configurar columnas
        headers = ["ID", "Nombre", "Contacto", "Teléfono", "Email", "Dirección", "Estado"]
    ...
    def load_suppliers(self):
        ...
        try:
            self.proveedores_cache = self.inventario_service.obtener_proveedores()
            self.update_suppliers_table()
            self.update_statistics()
            
        except Exception as e:
            logger.error(f"Error cargando proveedores: {e}")
            QMessageBox.warning(self, "Error", f"No se pudieron cargar los proveedores: {str(e)}")
    ...
    def update_suppliers_table(self):
        ...
        try:
            ...
            for row, proveedor in enumerate(self.proveedores_cache):
                # ID
                self.suppliers_table.setItem(row, 0, QTableWidgetItem(str(proveedor.get('id', ''))))
                
                # Nombre
                self.suppliers_table.setItem(row, 1, QTableWidgetItem(proveedor.get('nombre', '')))
                
                # Contacto
                self.suppliers_table.setItem(row, 2, QTableWidgetItem(proveedor.get('contacto', '')))
                
                # Teléfono
                self.suppliers_table.setItem(row, 3, QTableWidgetItem(proveedor.get('telefono', '')))
                
                # Email
                self.suppliers_table.setItem(row, 4, QTableWidgetItem(proveedor.get('email', '')))
                
                # Dirección
                self.suppliers_table.setItem(row, 5, QTableWidgetItem(proveedor.get('direccion', '')))
                ...
                self.suppliers_table.setItem(row, 6, item)
    ...
    def edit_selected_supplier(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Error", f"No se pudo editar el proveedor: {str(e)}")
    ...
    def delete_selected_supplier(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Error", f"No se pudo eliminar el proveedor: {str(e)}")
...
class SupplierDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("✏️ Editar Proveedor" if self.is_edit_mode else "➕ Nuevo Proveedor")
        self.setModal(True)
        self.resize(600, 500)
        ...
        self.name_input.setPlaceholderText("Nombre de la empresa o proveedor (requerido)")
        ...
        self.email_input.setPlaceholderText("Correo electrónico (ej: contacto@empresa.com)")
        ...
        form_layout.addWidget(self.address_input, row, 1)
        ...
        form_layout.addWidget(self.address_char_counter, row, 1, Qt.AlignmentFlag.AlignRight)
    ...
    def setup_validations(self):
        ...
        self.email_input.textChanged.connect(self.validate_email)        # Contador de caracteres para dirección
    ...
    def save_supplier(self):
        """Guardar el proveedor con validaciones completas"""
        ...
        try:
            ...
            if self.is_edit_mode and self.proveedor:
                ...
                success = self.inventario_service.actualizar_proveedor(
                    self.proveedor.get('id'), nombre, contacto, telefono, email, direccion
                )
                message = "Proveedor actualizado exitosamente" if success else "Error al actualizar el proveedor"
            else:
                # Crear nuevo proveedor
                success = self.inventario_service.crear_proveedor(
                    nombre, contacto, telefono, email, direccion
                )
                message = "Proveedor creado exitosamente" if success else "Error al crear el proveedor"
    ...
    def update_address_counter(self):
        ...
        if char_count > 270:
            self.address_char_counter.setStyleSheet("color: #dc3545; font-size: 11px; font-weight: bold;")
        elif char_count > 240:
            self.address_char_counter.setStyleSheet("color: #ffc107; font-size: 11px; font-weight: bold;")
        else:
            self.address_char_counter.setStyleSheet("color: #6c757d; font-size: 11px;")
