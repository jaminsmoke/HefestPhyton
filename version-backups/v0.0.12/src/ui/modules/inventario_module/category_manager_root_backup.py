...
from PyQt6.QtWidgets import (
    ...
    QLineEdit, QGroupBox, QGridLayout, QSpacerItem,
...
class CategoryManagerWidget(QWidget):
    ...
    def load_categories(self):
        ...
        try:
            self.categorias_cache = self.inventario_service.obtener_categorias()
    ...
    def update_categories_table(self):
        ...
        try:
            ...
            for row, categoria in enumerate(self.categorias_cache):
                # ID
                self.categories_table.setItem(row, 0, QTableWidgetItem(str(categoria.get('id', ''))))
                
                # Nombre
                self.categories_table.setItem(row, 1, QTableWidgetItem(categoria.get('nombre', '')))
                
                # Descripción
                descripcion = categoria.get('descripcion', '') or 'Sin descripción'
                self.categories_table.setItem(row, 2, QTableWidgetItem(descripcion))
                
                # Número de productos (simulado por ahora)
                productos_count = self.get_products_count_for_category(categoria.get('nombre', ''))
                self.categories_table.setItem(row, 3, QTableWidgetItem(str(productos_count)))
                
                # Fecha creada
                fecha_creada = categoria.get('fecha_creacion', 'N/A')
                if fecha_creada != 'N/A':
                    try:
                        fecha_creada = datetime.fromisoformat(fecha_creada).strftime('%Y-%m-%d')
                    except:
                        pass
                self.categories_table.setItem(row, 4, QTableWidgetItem(fecha_creada))
                
                # Fecha modificada
                fecha_modificada = categoria.get('fecha_modificacion', 'N/A')
                if fecha_modificada != 'N/A':
                    try:
                        fecha_modificada = datetime.fromisoformat(fecha_modificada).strftime('%Y-%m-%d')
                    except:
                        pass
                self.categories_table.setItem(row, 5, QTableWidgetItem(fecha_modificada))
    ...
    def get_products_count_for_category(self, categoria_nombre: str) -> int:
        ...
        try:
            productos = self.inventario_service.get_productos(categoria=categoria_nombre)
    ...
    def update_statistics(self):
        ...
        try:
            ...
            for categoria in self.categorias_cache:
                count = self.get_products_count_for_category(categoria.get('nombre', ''))
                if count > 0:
                    used += 1
                else:
                    empty += 1
    ...
    def on_category_selected(self):
        ...
        selected_rows = set(item.row() for item in self.categories_table.selectedItems())
        ...
        if has_selection:
            ...
            if row < len(self.categorias_cache):
                categoria = self.categorias_cache[row]
    ...
    def edit_selected_category(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Error", f"No se pudo editar la categoría: {str(e)}")
    ...
    def delete_selected_category(self):
        ...
        try:
            ...
            if row < len(self.categorias_cache):
                ...
                if productos_count > 0:
                    QMessageBox.warning(
                        ...
                        f"La categoría '{categoria.get('nombre', '')}' tiene {productos_count} producto(s) asociado(s). "
                ...
                reply = QMessageBox.question(
                    self,
                    "Confirmar eliminación",
                    f"¿Estás seguro de que deseas eliminar la categoría '{categoria.get('nombre', '')}'?",
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
                )
                
                if reply == QMessageBox.StandardButton.Yes:
                    if self.inventario_service.eliminar_categoria(categoria.get('id')):
                        self.load_categories()
                        self.categoria_actualizada.emit()
            ...
            QMessageBox.warning(self, "Error", f"No se pudo eliminar la categoría: {str(e)}")
...
class CategoryDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("✏️ Editar Categoría" if self.is_edit_mode else "➕ Nueva Categoría")
        self.setModal(True)
        self.resize(500, 350)
        ...
        self.name_input.setPlaceholderText("Ingrese el nombre de la categoría (requerido)")
        ...
        self.description_input.setPlaceholderText("Descripción detallada de la categoría (opcional)")
    ...
    def validate_name(self):
        ...
        if not name:
            ...
            self.name_error_label.setText("⚠️ El nombre debe tener al menos 2 caracteres")
    ...
    def update_char_counter(self):
        ...
        if char_count > 450:
            self.char_counter.setStyleSheet("color: #dc3545; font-size: 11px; font-weight: bold;")
        elif char_count > 400:
            self.char_counter.setStyleSheet("color: #ffc107; font-size: 11px; font-weight: bold;")
        else:
            self.char_counter.setStyleSheet("color: #6c757d; font-size: 11px;")
        ...
        try:
            ...
            if self.is_edit_mode and self.categoria:
                ...
                success = self.inventario_service.actualizar_categoria(
                    self.categoria.get('id'), nombre, descripcion
                )
                message = "Categoría actualizada exitosamente" if success else "Error al actualizar la categoría"
            else:
                # Crear nueva categoría
                success = self.inventario_service.crear_categoria(nombre, descripcion)
                message = "Categoría creada exitosamente" if success else "Error al crear la categoría"
    ...
    def validate_form(self):
        ...
        if not self.validate_name():
            is_valid = False
            error_messages.append("• El nombre es obligatorio y debe tener al menos 2 caracteres")
